...
PlotNE <- ggplot(NE_data, aes(x = Days, y = NE)) + 
  ...
  theme_classic() +
  labs(x="Days", y="NE") 
leveneTest(NE ~ Days, data=NE_data)
...
PlotCE <- ggplot(NE_data, aes(x = Days, y = CE)) + 
  ...
  theme_classic() +
  labs(x="Days", y="NE")
leveneTest(CE ~ Days, data=NE_data)
...
PlotSE <- ggplot(NE_data, aes(x = Days, y = SE)) + 
  ...
  theme_classic() +
  labs(x="Days", y="NE")
leveneTest(SE ~ Days, data=NE_data)
...
Since the variance of NE, CE, SE increases over time (heteroscedasticity in the residuals), we decided to use the function nlme::lme to make our ANOVAs, because the function allows to weight the variance by days. 
...
NE_meanse2<-data.frame(
  days=c("0", "20", "40", "60"),
  mean=as.vector(summary(modNE)$tTable[,1]) + as.vector(c(0, rep(0.03493554, 3))),
  se=as.vector(summary(modNE)$tTable[,2]),
  label=c("a", "b", "c", "c"))

NE_meanse2$ci<-NE_meanse2$se*1.96
...
plotdaniel<-ggarrange(tableGrob(NE_meanse), tableGrob(NE_meanse2), NEp, NEp2, ncol=2, nrow=2)
annotate_figure(plotdaniel, top=text("gauche: prédictions du modèle; droite: prédictions calculées à partir des données seulement"))
```
